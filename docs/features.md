1. In add alarm menu, remove "enabled" switch, it's confusing
2. Not clear how to set periodicity as its in the menu at the bottom and you need to scroll, if you don't scroll you don't see it available. 
Ascending alarm selection should impact preview
Preview is a stupid name
When you click on face, you should see the bottom dots/swipe up, swipe down (themes) options, now you need to guess
Would be good to add some audio feedback on some clicks, i.e. some nice click sound
Increase brightness with a sunrise in night mode
Annoying alarm feature:
Basic math
3+8, 14−6, maybe simple multiplication on higher difficulty.
Classic. Effective because it forces actual cognition.

night mode if disabled  settings should be greyed/disabled 

Language support (English, Polish, Russian, Ukrainian, Dutch, German, French, Italian)


3d printing suggestions/improvements:

For a 3D printed part, my first choice would be:
	•	add 0.3 mm interference on diameter
	•	plus 3 small retention bumps